...
namespace emacs
{
	public class EmacsEditor : IEditor
	{
		private Command _launchCommand = null;
		private int _port = 9997;
		...
		private void appendArguments(string[] arguments) {
            ...
            var portStr = args
                .Where(x => x.Key == "--editor.emacs.port")
				.Select(x => x.Value)
				.FirstOrDefault();
            if (portStr != null) {
                int port;
