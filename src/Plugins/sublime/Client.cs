...
namespace sublime 
{
	public class Client
	{
            ...
            try
            {
                ...
                for (var i = 0; i < x;i++)
                {
                    if (isEndOfMessage(i))
                    {
                        ...
                        ThreadPool.QueueUserWorkItem((m) => _onMessage(actual), null);
                        _readBuffer.SetLength(0);
                    }
                    else
                    {
		...
		public string Request(string message, string waitFor)
		{
			var originalMessageHandler = _onMessage;
			string recieved= null;
			Action<string> msgHandler = (m) => {
					if (m == waitFor)
						recieved = m;
					else
						originalMessageHandler(m);
				};
			_onMessage = msgHandler;
			Send(message);
			var timeout = DateTime.Now;
			...
			_onMessage = originalMessageHandler;
			return recieved;
		}
