...
namespace EditorEngine.Core.Tests.Commands.Handlers
{
	...
	public class GoToHandlerTests
	{
		...
		{
			_handler.Execute(msg("not valid argument"));
		...
		{
			_handler.Execute(msg(string.Format("{0}.NotExisting", Assembly.GetExecutingAssembly().Location)));
			_dispatcher.Published<UsageErrorMessage>();
		}
		...
		{
			_handler.Execute(msg(string.Format("{0}|{1}|{2}", Assembly.GetExecutingAssembly().Location, 10, 15)));
		...
		{
			_handler.Execute(msg(string.Format("{0}|12", Assembly.GetExecutingAssembly().Location)));
			var messsage = _dispatcher.GetPublishedMessage<EditorGoToMessage>();
		...
		{
			_handler.Execute(msg(string.Format("{0}|0|7", Assembly.GetExecutingAssembly().Location)));
			var messsage = _dispatcher.GetPublishedMessage<EditorGoToMessage>();
		...
		{
			return new CommandMessage(Guid.Empty, "goto " + args);
		}
	}
}
